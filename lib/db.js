 ...
 * that specific match so it doesn't sit forever in "pending scores" waiting for a game
 * that's never going to finish. Past, already-scored results are untouched. */
export async function checkOutPlayer(eventId, player, matches) {
  const pendingMatch = matches.find(
    (m) => m.status === "scheduled" && [...m.team_a, ...m.team_b].includes(player.id)
  );

  if (pendingMatch) {
    const { error } = await supabase.from("matches").update({ status: "cancelled" }).eq("id", pendingMatch.id);
    if (error) throw error;
    await logEvent(
      eventId,
      `Court ${pendingMatch.court} (${pendingMatch.division === "female" ? "Women's" : "Men's"} Doubles) cancelled -- ${player.display_name} checked out mid-match`
    );
  }
...
export async function submitScore(match, scoreA, scoreB, players, status = "completed") {
  ...
  await logEvent(
    match.event_id,
    `Court ${match.court} (${match.division === "female" ? "Women's" : "Men's"} Doubles): ${teamAName} vs ${teamBName} \u2014 ${scoreA}-${scoreB}`
  );
}
