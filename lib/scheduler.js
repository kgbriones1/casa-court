 ...
 */
/** Splits checked-in players (minus any hard-reserved ids) into per-gender pools that
 * are actually eligible to play this round, applying the same rest-gap relaxation
 * generateDraft uses internally: excluded by default, but relaxed back to the full
 * gender pool (with a warning) if strict exclusion would leave fewer than 4 available.
 * Callers computing a court split (suggestCourtSplit) MUST use this instead of a raw
 * "!justPlayedIds.has(id)" filter, or the split will be calculated against a pool
 * generateDraft doesn't actually use once relaxation kicks in -- see the regression
 * suite in scheduler.test.js for what that mismatch does to pacing and partner reuse. */
export function eligiblePlayersByGender(allPlayers, futureReservedIds = new Set(), justPlayedIds = new Set()) {
  ...
  return { eligibleByGender, warnings };
}
...
export function generateDraft(allPlayers, numCourts, roundNumber, futureReservedIds = new Set(), justPlayedIds = new Set(), courtsForGender = null) {
  ...
  const { eligibleByGender, warnings } = eligiblePlayersByGender(allPlayers, futureReservedIds, justPlayedIds);
