...
/** Computes the post-cancellation assignment-history state for a match's 4 players --
 * a cancelled match never happened, so it shouldn't keep counting against future
 * repeat-partner/opponent avoidance or rest-gap timing. Removes exactly one
 * occurrence of the partnership from partner_ids (not all -- they may have
 * legitimately partnered again in a different round), decrements opponent_counts
 * by 1 for each opponent (dropping the key at 0), and recomputes last_played_round
 * as the highest round number among the player's other non-cancelled matches (or
 * null if this was their only one). Pure -- returns plain {id, display_name,
 * partner_ids, opponent_counts, last_played_round} objects for exactly the 4
 * touched players, doesn't mutate its inputs.
 * @param {object} match - the match being cancelled ({id, team_a, team_b, ...})
 * @param {Array} players - every player row for the event
 * @param {Array} roundsWithMatches - [{number, matches}], each match having at
 *   least {id, status, team_a, team_b} */
export function reverseMatchHistory(match, players, roundsWithMatches) {
  ...
  return touched.map((id) => {
    ...
    const otherRoundNumbers = roundsWithMatches
      .filter((r) => r.matches.some((m) => m.id !== match.id && m.status !== "cancelled" && [...m.team_a, ...m.team_b].includes(id)))
      .map((r) => r.number);
    const last_played_round = otherRoundNumbers.length ? Math.max(...otherRoundNumbers) : null;

    return { id, display_name: p.display_name, partner_ids, opponent_counts, last_played_round };
  });
}
