...
import { generateDraft, eligiblePlayers, matchTypeLabel, reverseMatchHistory } from "./scheduler.js";
...
describe("reverseMatchHistory", () => {
  test("removes exactly one partnership occurrence, not all of them", () => {
    ...
    a.partner_ids = ["b", "b"]; // partnered with b twice, in two different rounds
    ...
    d.partner_ids = ["c"];
    const match = { id: "m1", court: 1, team_a: ["a", "b"], team_b: ["c", "d"] };
  ...
  test("recomputes last_played_round as the highest round among the player's other non-cancelled matches", () => {
    const players = [makePlayer("a", "female"), makePlayer("b", "female"), makePlayer("c", "male"), makePlayer("d", "male")];
    players.forEach((p) => { p.last_played_round = 5; });
    const cancelledMatch = { id: "m5", court: 1, team_a: ["a", "b"], team_b: ["c", "d"] };
    const roundsWithMatches = [
      { number: 3, matches: [{ id: "m3", status: "completed", team_a: ["a", "c"], team_b: ["b", "d"] }] },
      { number: 5, matches: [cancelledMatch] },
    ];

    const reversed = reverseMatchHistory(cancelledMatch, players, roundsWithMatches);
    const byId = Object.fromEntries(reversed.map((p) => [p.id, p]));
    expect(byId.a.last_played_round).toBe(3); // played round 3 too, so falls back to that
    expect(byId.b.last_played_round).toBe(3);
  });

  test("last_played_round becomes null when the cancelled match was the player's only one", () => {
    const players = [makePlayer("a", "female"), makePlayer("b", "female"), makePlayer("c", "male"), makePlayer("d", "male")];
    players.forEach((p) => { p.last_played_round = 1; });
    const cancelledMatch = { id: "m1", court: 1, team_a: ["a", "b"], team_b: ["c", "d"] };
    const roundsWithMatches = [{ number: 1, matches: [cancelledMatch] }];

    const reversed = reverseMatchHistory(cancelledMatch, players, roundsWithMatches);
    reversed.forEach((p) => expect(p.last_played_round).toBeNull());
  });

  test("ignores other cancelled matches when recomputing last_played_round", () => {
    const players = [makePlayer("a", "female"), makePlayer("b", "female"), makePlayer("c", "male"), makePlayer("d", "male")];
    const cancelledMatch = { id: "m2", court: 1, team_a: ["a", "b"], team_b: ["c", "d"] };
    const roundsWithMatches = [
      { number: 1, matches: [{ id: "m1", status: "cancelled", team_a: ["a", "c"], team_b: ["b", "d"] }] },
      { number: 2, matches: [cancelledMatch] },
    ];
